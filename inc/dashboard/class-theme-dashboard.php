...
class Base_Dashboard_Settings {
	...
	public function add_menu() {
		$page = add_theme_page( __( 'jmsiteam - Next Generation Theme', 'jmsiteam' ), __( 'jmsiteam', 'jmsiteam' ), apply_filters( 'base_admin_settings_capability', 'manage_options' ), 'jmsiteam', array( $this, 'config_page' ) );
	...
	public function scripts() {
		...
		$button_label = esc_html__( 'Browse Base Starter Templates', 'jmsiteam' );
		$data_action  = '';
		...
		wp_enqueue_style( 'base-dashboard', get_template_directory_uri() . '/inc/dashboard/react/dash-controls.min.css', array( 'wp-components' ), jmsiteam_VERSION );
		wp_enqueue_script( 'base-dashboard', get_template_directory_uri() . '/assets/js/admin/dashboard.js', array( 'wp-i18n', 'wp-element', 'wp-plugins', 'wp-components', 'wp-api', 'wp-hooks', 'wp-edit-post', 'lodash', 'wp-block-library', 'wp-block-editor', 'wp-editor', 'jquery' ), jmsiteam_VERSION, true );
		wp_localize_script(
			'base-dashboard',
			'baseDashboardParams',
			array(
				'adminURL' => esc_url( admin_url() ),
				'settings' => esc_attr( get_option( 'jmsiteam_theme_config' ) ),
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'base-dashboard', 'jmsiteam' );
		}
	}
	/**
	...
	public function load_api_settings() {
		...
		register_setting(
			'jmsiteam_theme_config',
			'jmsiteam_theme_config',
			array(
				'type'              => 'string',
				'description'       => __( 'Config Theme Modules', 'jmsiteam' ),
			...
			<div class="base_theme_dash_head">
				<div class="base_theme_dash_head_container">
					<div class="base_theme_dash_logo">
						<img src="<?php echo esc_attr( get_template_directory_uri() . '/assets/images/jmsiteam-logo.png' ); ?>">
					</div>
					<div class="base_theme_dash_version">
						<span>
							<?php echo esc_html( jmsiteam_VERSION ); ?>
						</span>
					</div>
				</div>
			</div>
					...
					<div class="side-panel">
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Video Tutorials', 'jmsiteam' ); ?></h2>
								<p><?php esc_html_e( 'Want a guide? We have video tutorials to walk you through getting started.', 'jmsiteam' ); ?></p>
								<a href="https://jmsiteam.org/video-tutorials" target="_blank" class="sidebar-link"><?php esc_html_e( 'Watch Videos', 'jmsiteam' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Documentation', 'jmsiteam' ); ?></h2>
								<p><?php esc_html_e( 'Need help? We have a knowledge base full of articles to get you started.', 'jmsiteam' ); ?></p>
								<a href="https://jmsiteam.org/kb" target="_blank" class="sidebar-link"><?php esc_html_e( 'Browse Docs', 'jmsiteam' ); ?></a>
							</div>
						</div>
						<div class="support-section sidebar-section components-panel">
							<div class="components-panel__body is-opened">
								<h2><?php esc_html_e( 'Support', 'jmsiteam' ); ?></h2>
								<p><?php esc_html_e( 'Have a question, we are happy to help! Get in touch with our support team.', 'jmsiteam' ); ?></p>
								<a href="https://jmsiteam.org/support" target="_blank" class="sidebar-link"><?php esc_html_e( 'Submit a Ticket', 'jmsiteam' ); ?></a>
							</div>
						</div>
					</div>
				</div>
