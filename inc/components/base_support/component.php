...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function base_welcome_notice() {
		if ( ! defined( 'jmsiteam_VERSION' ) || get_option( 'base_disable_welcome_notice' ) ) {
			return;
		}
		?>
		<div id="base-notice-welcome-plugin" class="notice base-notice is-dismissible notice-info">
			<h3 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the jmsiteam Theme!', 'jmsiteam' ); ?></h3>
			<p class="base-notice-description"><?php echo esc_html__( 'You can customize the styling of your theme - including header fooder layout, font, colors and spacing - in your theme settings.', 'jmsiteam' ); ?></p>
			<?php if ( !isset( $_GET['page'] ) || $_GET['page'] !== 'jmsiteam' ) { ?>
			<p class="install-submit">
				<a href="<?php echo esc_url( admin_url( 'themes.php?page=jmsiteam' ) ); ?>"><button class="button button-primary base-theme-settings"><?php echo esc_html__( 'Go to Theme Settings', 'jmsiteam' ); ?></button></a>
			</p>
			<?php } ?>
		</div>
		<?php
	...
	public function base_starter_templates_notice() {
		...
		if ( ! isset( $installed_plugins['base-starter/base-starter.php'] ) ) {
			$button_label = esc_html__( 'Install Base Starter Templates', 'jmsiteam' );
			$data_action  = 'install';
		} elseif ( ! self::active_plugin_check( 'base-starter/base-starter.php' ) ) {
			$button_label = esc_html__( 'Activate Base Starter Templates', 'jmsiteam' );
		...
		<div id="base-notice-starter-templates" class="notice base-notice is-dismissible notice-info">
			<h3 class="notice-title"><?php echo esc_html__( 'Thanks for choosing the jmsiteam Theme!', 'jmsiteam' ); ?></h3>
			<p class="base-notice-description"><?php /* translators: %s: <strong> <a> */ printf( esc_html__( 'Want to get started with a beautiful %1$sstarter template%2$s? Install the Base Starter Templates plugin to launch an optimized design in minutes.', 'jmsiteam' ), '<a href="https://wordpress.org/plugins/base-starter-templates/" target="_blank">', '</a>', '<strong>', '</strong>' ); ?></p>
			<p class="install-submit">
				<button class="button button-primary base-install-starter-btn" data-redirect-url="<?php echo esc_url( admin_url( 'themes.php?page=base-starter' ) ); ?>" data-activating-label="<?php echo esc_attr__( 'Activating...', 'jmsiteam' ); ?>" data-activated-label="<?php echo esc_attr__( 'Activated', 'jmsiteam' ); ?>" data-installing-label="<?php echo esc_attr__( 'Installing...', 'jmsiteam' ); ?>" data-installed-label="<?php echo esc_attr__( 'Installed', 'jmsiteam' ); ?>" data-action="<?php echo esc_attr( $data_action ); ?>"><?php echo esc_html( $button_label ); ?></button>
			</p>
		</div>
		<?php
	...
	public function base_turn_off_gutenberg_plugin_notice() {
		...
		<div id="base-notice-gutenberg-plugin" class="notice base-notice is-dismissible notice-warning">
			<h3 class="notice-title"><?php echo esc_html__( 'Gutenberg Plugin Detected', 'jmsiteam' ); ?></h3>
			<p class="base-notice-description"><?php /* translators: %s: <a> */ printf( esc_html__( 'The %1$sGutenberg plugin%2$s is not recommended for use in a production site. Many things may be broken by using this plugin. Please deactivate.', 'jmsiteam' ), '<a href="https://wordpress.org/plugins/gutenberg/" target="_blank">', '</a>' ); ?></p>
		</div>
		<?php
	...
	public function install_starter_script() {
		...
		wp_register_script( 'base-welcome-deactivate', get_template_directory_uri() . '/assets/js/welcome-notice.min.js', array( 'jquery' ), jmsiteam_VERSION, false );
		wp_register_script( 'base-starter-install', get_template_directory_uri() . '/assets/js/admin-activate.min.js', array( 'jquery' ), jmsiteam_VERSION, false );
		wp_register_script( 'base-gutenberg-deactivate', get_template_directory_uri() . '/assets/js/gutenberg-notice.min.js', array( 'jquery' ), jmsiteam_VERSION, false );
	}
	/**
	 * Active Plugin Check
	 *
	...
	public function install_plugin_ajax_callback() {
		if ( ! check_ajax_referer( 'base-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, Please reload the page.', 'jmsiteam' ) );
		}
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Security Error, Need higher Permissions to install plugin.', 'jmsiteam' ) );
		}
		...
		if ( false === $install ) {
			wp_send_json_error( __( 'Error, plugin could not be installed, please install manually.', 'jmsiteam' ) );
		} else {
	...
	public static function customizer_quick_link() {
		if ( is_customize_preview() ) {
			...
			<div class="customize-partial-edit-shortcut base-custom-partial-edit-shortcut">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'jmsiteam' ); ?>" title="<?php esc_attr_e( 'Click to edit this element.', 'jmsiteam' ); ?>" class="customize-partial-edit-shortcut-button item-customizer-focus"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"></path></svg></button>
			</div>
			<?php
		}
	}
