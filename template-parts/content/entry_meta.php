...
if ( isset( $elements ) && is_array( $elements ) ) {
	...
	if ( isset( $elements['author'] ) && $elements['author'] ) {
		...
		if ( isset( $elements['authorEnableLabel'] ) && $elements['authorEnableLabel'] ) {
			$meta_labels['author'] = ( isset( $elements['authorLabel'] ) && ! empty( $elements['authorLabel'] ) ? $elements['authorLabel'] : __( 'By', 'jmsiteam' ) );
		}
	...
	if ( isset( $elements['date'] ) && $elements['date'] ) {
		...
		if ( isset( $elements['dateEnableLabel'] ) && $elements['dateEnableLabel'] ) {
			$meta_labels['date'] = ( isset( $elements['dateLabel'] ) && ! empty( $elements['dateLabel'] ) ? $elements['dateLabel'] : __( 'Posted on', 'jmsiteam' ) );
		}
	}
	if ( isset( $elements['dateUpdated'] ) && $elements['dateUpdated'] ) {
		...
		if ( isset( $elements['dateUpdatedEnableLabel'] ) && $elements['dateUpdatedEnableLabel'] ) {
			$meta_labels['dateUpdated'] = ( isset( $elements['dateUpdatedLabel'] ) && ! empty( $elements['dateUpdatedLabel'] ) ? $elements['dateUpdatedLabel'] : __( 'Updated on', 'jmsiteam' ) );
		}
	}
	if ( isset( $elements['categories'] ) && $elements['categories'] ) {
		...
		if ( isset( $elements['categoriesEnableLabel'] ) && $elements['categoriesEnableLabel'] ) {
			$meta_labels['categories'] = ( isset( $elements['categoriesLabel'] ) && ! empty( $elements['categoriesLabel'] ) ? $elements['categoriesLabel'] : __( 'Posted in', 'jmsiteam' ) );
		}
	}
						...
						if ( $author_link ) {
							if ( get_the_author_meta( 'url' ) && apply_filters( 'base_author_use_profile_link', true ) ) {
								$author_output .= sprintf(
									...
									esc_url( get_the_author_meta( 'url', $author_id ) ),
									esc_attr__( 'Author image', 'jmsiteam' ),
			...
			case 'comments':
				...
				if ( $show ) {
					...
					if ( '1' === get_comments_number() ) {
						echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comment', 'jmsiteam' );
					} else {
						echo esc_html( get_comments_number() ) . ' ' . esc_html__( 'Comments', 'jmsiteam' );
					}
					echo '</a>';
					echo '</div>';
				}
